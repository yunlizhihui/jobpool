...
import (
	...
	"sync/atomic"
	"time"
	"yunli.com/jobpool/api/v2/authpb"
...
// 通用的分页方法
func GetPage(data interface{}, request PageRequest) *PageResponse {
	// 初始化分页参数
	v := reflect.ValueOf(data)
	length := int64(v.Len())
	pageNumber := getPageNumber(request.PageNumber)
	pageSize := getPageSize(request.PageSize)
	// 获取分页开始索引和结束索引
	startIndex := getStartIndex(pageNumber, pageSize, length)
	endIndex := getEndIndex(pageNumber, pageSize, length)
	// 切片元素的类型
	elementType := v.Type().Elem()
	result := reflect.MakeSlice(reflect.SliceOf(elementType), 0, 0)

	for i := 0; i < v.Len(); i++ {
		if checkStartIndex(int64(i), startIndex) {
			continue
		}
		if checkEndIndex(int64(i), endIndex) {
			break
		}
	...
	response := &PageResponse{
		...
		TotalElements: int32(length),
		TotalPages:    int32(getTotalPages(length, pageSize)),
	}
	return response
}

// 判断索引是否小于开始索引
func checkStartIndex(index int64, startIndex int64) bool {
	return index < startIndex
}

// 判断索引是否大于结束索引
func checkEndIndex(index int64, endIndex int64) bool {
	return index >= endIndex
}

// 获取默认分页页数参数
func getPageNumber(pageNumber int32) int64 {
	if pageNumber == 0 {
		return 1
	}
...
// 获取默认分页size参数
func getPageSize(pageSize int32) int64 {
	if pageSize == 0 {
...
// 获取分页开始索引
func getStartIndex(pageNumber int64, pageSize int64, totalElements int64) int64 {
...
// 获取分页结束索引
func getEndIndex(pageNumber int64, pageSize int64, totalElements int64) int64 {
...
// 获取总页数
func getTotalPages(totalElements int64, pageSize int64) int64 {
...
func (as *authStore) UserList(r *pb.AuthUserListRequest) (*pb.AuthUserListResponse, error) {
	...
	// 分页操作
	pageResponse := GetPage(users, PageRequest{
		PageNumber: r.PageNumber,
		PageSize:   r.PageSize,
	})
	data := make([]string, reflect.ValueOf(pageResponse.Data).Len())
	//返回值类型转换
	for i := 0; i < reflect.ValueOf(pageResponse.Data).Len(); i++ {
		data[i] = string(pageResponse.Data.([]*authpb.User)[i].Name)
	}
	resp := &pb.AuthUserListResponse{
		Users:         data,
...
func (as *authStore) RoleList(r *pb.AuthRoleListRequest) (*pb.AuthRoleListResponse, error) {
	...
	// 分页操作
	pageResponse := GetPage(roles, PageRequest{
		PageNumber: r.PageNumber,
		PageSize:   r.PageSize,
	})
	data := make([]string, reflect.ValueOf(pageResponse.Data).Len())
	//返回值类型转换
	for i := 0; i < reflect.ValueOf(pageResponse.Data).Len(); i++ {
		data[i] = string(pageResponse.Data.([]*authpb.Role)[i].Name)
	}
	resp := &pb.AuthRoleListResponse{
		Roles:         data,
		TotalElements: pageResponse.TotalElements,
		TotalPages:    pageResponse.TotalPages,
	}
