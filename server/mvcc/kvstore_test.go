...
func TestIndex(t *testing.T) {
	key := fmt.Sprintf("%s_%s", "default", "6a052b8a-3e52-1e9c-a3fb-18887a550f4e")
	match := "default"
	a := []byte(key)
	b := []byte(match)
	ind := bytes.Index(a, b)
...
func (b *fakeBatchTx) UnsafeRangeStart(bucket backend.Bucket, key []byte, limit int64) (keys [][]byte, vals [][]byte) {
	b.Recorder.Record(testutil.Action{Name: "rangeStart", Params: []interface{}{bucket, key, limit}})
	r := <-b.rangeRespc
	return r.keys, r.vals
}
