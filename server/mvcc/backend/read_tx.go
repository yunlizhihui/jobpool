...
type ReadTx interface {
	...
	UnsafeRangeStart(bucket Bucket, key []byte, limit int64) (keys [][]byte, vals [][]byte)
}
...
func (baseReadTx *baseReadTx) UnsafeRangeStart(bucketType Bucket, key []byte, limit int64) ([][]byte, [][]byte) {
	if limit <= 0 {
		limit = math.MaxInt64
	}
	if limit > 1 && !bucketType.IsSafeRangeBucket() {
	...
	keys, vals := baseReadTx.buf.RangeStart(bucketType, key, limit)
	if int64(len(keys)) == limit {
		return keys, vals
	}
	...
	k2, v2 := unsafeRangeStart(c, key, limit-int64(len(keys)))
	return append(k2, keys...), append(v2, vals...)
}
