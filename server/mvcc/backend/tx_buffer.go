...
func (txr *txReadBuffer) RangeStart(bucket Bucket, key []byte, limit int64) ([][]byte, [][]byte) {
	if b := txr.buckets[bucket.ID()]; b != nil {
		return b.RangeStart(key, limit)
	}
	return nil, nil
}
...
func (bb *bucketBuffer) RangeStart(key []byte, limit int64) (keys [][]byte, vals [][]byte) {
	f := func(i int) bool { return bytes.Index(bb.buf[i].key, key) >= 0 }
	idx := sort.Search(bb.used, f)
	if idx < 0 {
		return nil, nil
	}
	if bytes.Index(bb.buf[idx].key, key) < 0 {
		return nil, nil
	}
	for i := idx; i < bb.used && int64(len(keys)) < limit; i++ {
		if bytes.Index(bb.buf[i].key, key) < 0 {
			break
		}
		keys = append(keys, bb.buf[i].key)
		vals = append(vals, bb.buf[i].val)
