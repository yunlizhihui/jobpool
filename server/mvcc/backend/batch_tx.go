...
func (t *batchTx) UnsafeRangeStart(bucketType Bucket, keyStart []byte, limit int64) ([][]byte, [][]byte) {
	bucket := t.tx.Bucket(bucketType.Name())
	if bucket == nil {
		t.backend.lg.Fatal(
			"failed to find a bucket",
	...
	return unsafeRangeStart(bucket.Cursor(), keyStart, limit)
}

func unsafeRangeStart(c *bolt.Cursor, keyStart []byte, limit int64) (keys [][]byte, vs [][]byte) {
	if limit <= 0 {
		limit = math.MaxInt64
	}
	var isMatch func(b []byte) bool
	isMatch = func(b []byte) bool { return bytes.Index(b, keyStart) >= 0 }

	for ck, cv := c.Seek(keyStart); ck != nil && isMatch(ck); ck, cv = c.Next() {
		vs = append(vs, cv)
		keys = append(keys, ck)
