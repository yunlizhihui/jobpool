...
ROOT_MODULE="yunli.com/jobpool"

if [[ "$(go list)" != "${ROOT_MODULE}/v2" ]]; then
  echo "must be run from '${ROOT_MODULE}/v2' module directory"
  exit 255
fi

function set_root_dir {
  script_dir=$(dirname $(readlink -f $0))
  ETCD_ROOT_DIR=${script_dir%/*}
}
...
function tool_get_bin {
  ...
  # remove the version suffix, such as removing "/v3" from "yunli.com/jobpool/v2".
  local cmd_base_name
